...
if (!$conf['enableprofile']) {
    $text .= "<div class='text-center'><br />"._PROFILEDISABLED."<br /><br /></div>";
	require_once(FOOTERF);
	exit;
}
if(!$conf['profiletoguests'] && !USER && !ADMIN){
    $text .= "<div class='text-center'><br />"._RESTRICTEDGUESTS."<br /><br /></div>";
	//header("Refresh:1;URL=modulesx.php?name=$module_name");
	require_once(FOOTERF);
	exit;
}
...
$text = "<table class='table'
  <tr>
    <td class='text-right'><a href='clanmembers.php'>"._RETURNTOLIST."</a></td>
  </tr>
</table>";

//container is already in theme
$text .=   
"<div class='containerx'>
<div class='row'>
<div class='col-md-9 col-sm-8'> 
  
<table  class='fborder table memberinfo '>";
...
switch($infoname){
		...
		case "Gender":
			if($gender !="")
			$infos =  "$gender <img src='images/Profile/$gender.png' class='absmiddle'>";
		break;
		case "Age":
			$infos =  $age;
		break;
		...
		case "Steam ID":
			...
			$infos =  "<a href='http://vacbanned.com/view/detail/id/$steam' target='_blank'>$steam</a>";
		break;
		case "MSN":
			if($showcontact && $msn !=""){
		...
		case "Activity":			
			if($active == 1){
				$infos = "<span style='color:#008000;'>"._ACTIVE."</span>";
			}else{
				$infos = "<span style='color:#FF0000;'>"._INACTIVE."</span>";
			}
		break;
		case "Tryout":			
    ...
    break;
		...
		case "Wars Played":
			if(isset($pref['plug_installed']['clanwars'])){
				$wars_played = e107::getDb()->count("clan_wars_lineup", "(*)", "WHERE member='$memberid' AND available='2'");
				if($wars_played > 0)
				$infos = $wars_played;
			}
		break;
		case "Last War":
			if(isset($pref['plug_installed']['clanwars'])){
				...
				$wid = $row5['wid'];
				if($opp_tag !="")
				//$infos = "<a href='modulesx.php?name=Wars&op=Details&wid=$wid'>$opp_tag</a>";	
        $infos = "<a href='".e_PLUGIN."clanwars/clanwars.php?Details&wid=$wid'>$opp_tag</a>";							
			}
		break;
		//End Wars
}
...
$text .= "</table></div><div class='col-md-3 col-sm-4'>";
...
$text .= "<table class='table memberinfo'>
<tr><td class='forumheader2'><a href='$url' id='avatar'>
<img src='$url' $wihei  class='img-responsive' ></a></td>";
}
$text .= "</tr></table>";
...
if($conf['enablehardware']){
	...
	if($contents !=""){
		$text = "<table class='fborder table memberinfo '>";
		$text .= $contents;
		$text .= "</table>";
...
if($sql->db_Rows() > 0 && $conf['showawards']){
	$text = "<table  class='table' id='awardstable'>";
	while($row = $sql->db_Fetch()){
		$title = $row['title'];
...
if(USERID == $memberid){
	if($conf['allowchangeinfo']){
		echo "<br /><div class='text-center'><a href='clanmembers.php?editinfo'>"._CHANGEURINFO."</a><br /></div>";
	}elseif(VisibleInfo("User Image") && $conf['allowupimage']){			
		echo "<br /><div class='text-center'><a href='clanmembers.php?editinfo'>"._UPLOADURIMAGE."</a><br /></div>";
	}
}
if(ADMIN){
	echo "<br /><div class='text-center'><b>"._CMADMIN.":</b> <a href='admin_old.php?editmember&memberid=$memberid'>"._EDITMEMBER."</a><br /><br /></div>";
}
