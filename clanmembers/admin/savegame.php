...
//Update Members
$members = $sql->retrieve("clan_members_info", "userid", "Order BY userid ASC", true);

foreach($members as $row){
	if(intval($_POST["add".$row['userid']]) == 1){
     
		$match = $sql->count("clan_members_gamelink", "(*)", "WHERE userid='".$row['userid']."' and gid='$gid'");
		if($match == 0){
			$sql->insert("clan_members_gamelink", array("gid" => $gid, "userid" => $row['userid']));
		}
	}else{
     
		$sql->delete("clan_members_gamelink", "gid='$gid' and userid='".$row['userid']."'");
	}
