...
$result = $sql->retrieve("SELECT i.*, u.user_name from #clan_members_info i, #user u WHERE u.user_id=i.userid and i.userid='$memberid' LIMIT 1", true);
  $row = $result[0];
 ...
 // $votedate = (intval($row['votedate']) > 0 ? date("Y/m/d", $row['votedate']) : "");
  $votedate = (intval($row['votedate']) > 0 ?  $row['votedate'] : "");
...
echo "<form name='memberinfo' method='post' action='admin_old.php?savemember' enctype=\"multipart/form-data\">";
	...
	$text .= "<tr>
		...
		if($sql->count("clan_members_ranks") > 0){
		$text .= "<tr>
...
$newvotedate = e107::getForm()->datepicker('votedate', $votedate,  
  ...
  if($sql->count("clan_members_ranks") > 0){
		$text .= "<tr>
				<td align='left' width='120'>Member "._INFORank.":&nbsp;</td>
				<td align='left'><select name='memberrank'>";
				$text .= "<option value='0'>"._NORANK."</option>";
	...
	$ns->tablerender("<div id='general'>
			<div class='jm-collapse'>Edit the info of $member -</div>
			<div class='jm-expand'>Edit the info of $member +</div></div>", $text);
	...
	if(VisibleInfo("User Image")){
		...
		$ns->tablerender("<div id='uimage'>
				...
				<div class='jm-expand'>"._INFOUserImage." +</div></div>", $text);
	}
	if($conf['enableprofile'] && $conf['enablehardware']){
		...
		$ns->tablerender("<div id='hardware'>
					...
					<div class='jm-expand'>"._HARDWPROFILE." +</div></div>", $text);
...
if($games > 0){
	...
	$games = e107::getDB()->retrieve("clan_games", "*", "inmembers='1' order by position ASC", true);
	foreach($games as $row){
		...
		if($conf['rank_per_game'] == 1 && (VisibleInfo("Rank") or VisibleInfo("Rank Image")) && $conf['gamesorteams'] == "Games"){
			...
			$text .= "<option value='0'>"._NORANK."</option>";
			$ranks = e107::getDB()->retrieve("clan_members_ranks", "*", " ORDER BY rankorder", true);
				foreach($ranks AS $rowrank){
					$rid = $rowrank['rid'];
					$rank = $rowrank['rank'];
		
					//Match
					$rowmatch = e107::getDB()->retrieve("clan_members_gamelink", "*", "userid='$memberid' and gid='$gid'"  );					 
	...
	$ns->tablerender("<div id='games'>
			<div class='jm-collapse'>"._INFOGames." -</div>
			<div class='jm-expand'>"._INFOGames." +</div></div>", $text);
}
...
if($teams > 0){
	...
	$result = $sql->retrieve("clan_teams", "*", "inmembers='1' order by position ASC", true);
...
$text .= "<option value='0'>"._NORANK."</option>";
		...
		$ranks = $sql->retrieve("clan_members_ranks", "*", "ORDER BY rankorder", true);
			foreach($ranks AS $rowrank){
				$rid = $rowrank['rid'];
				$rank = $rowrank['rank'];
			...
			$rowmatch = e107::getDB()->retrieve("clan_members_teamlink", "*", "userid='$memberid' and tid='$tid' LIMIT 1 ");
	...
	$ns->tablerender("<div id='teams'>
			<div class='jm-collapse'>"._INFOTeams." -</div>
			<div class='jm-expand'>"._INFOTeams." +</div></div>", $text);
}
...
$ns->tablerender("<div id='awards'>
			<div class='jm-collapse'>"._AWARDS." -</div>
			<div class='jm-expand'>"._AWARDS." +</div></div>", $text);
