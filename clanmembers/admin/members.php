...
$rows =	$sql->retrieve("SELECT i.*, u.user_name from #clan_members_info i, #user u WHERE u.user_id=i.userid order by u.user_name", true);
		foreach($rows AS $row){
			...
			// members rank display 
			$rank = $sql->retrieve("clan_members_ranks", "rank", "rid='$rank'");
			...
			$text .= "<tr id='member$memberid'>
					...
					$text .= "<td class='forumheader3 nowrap' width='10' nowrap><input type='button' class='button' value='"._EDIT."' onclick=\"window.location='admin_old.php?editmember&memberid=$memberid'\">&nbsp;<input type='button' class='button' value='"._DEL."' onclick=\"DelMember('$memberid','$member');\"></td>
					</tr>";
			}
			$text .="</table>";
