...
$membersinfo = $sql->retrieve("clan_members_info", "userid", "Order BY userid ASC", true);
foreach($membersinfo as $row){
	if(intval($_POST["add".$row['userid']]) == 1){
		$match = e107::getDB()->count("clan_members_teamlink", "(*)", "WHERE userid='".$row['userid']."' and tid='$tid'");
		if($match == 0){
			e107::getDB()->insert("clan_members_teamlink", array("tid" => $tid, "userid" => $row['userid']));
		}
	}else{
		e107::getDB()->delete("clan_members_teamlink", "tid='$tid' and userid='".$row['userid']."'");
	}
