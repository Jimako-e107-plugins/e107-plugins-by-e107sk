...
include_lan(CLAN_FORUM_LANGUAGES.'lan_forum_stats.php');
...
$total_posts = $sql -> db_Count(CLAN_FORUM_THREAD_NOPREFIX);
$total_topics = $sql -> db_Count(CLAN_FORUM_THREAD_NOPREFIX, "(*)", "WHERE thread_parent=0");
$total_replies = $sql -> db_Count(CLAN_FORUM_THREAD_NOPREFIX, "(*)", "WHERE thread_parent!=0");
$total_views = 0;
$query = "SELECT sum(thread_views) AS total FROM ".CLAN_FORUM_THREAD." ";		// Try this way - original way didn't work for some users
if ($sql -> gen($query))
//if($sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX, "sum(thread_views) AS total", '', 'nowhere'))
{
  $row = $sql->fetch();
  $total_views = $row['total'];
}

$firstpost = $sql -> select(CLAN_FORUM_THREAD_NOPREFIX, "thread_datestamp", "thread_datestamp > 0 ORDER BY thread_datestamp ASC LIMIT 0,1");
$fp = $sql -> fetch();
...
//$query = "SHOW TABLE STATUS FROM `{$sql->mySQLdefaultdb}` LIKE '".MPREFIX.CLAN_FORUM_THREAD_NOPREFIX."' ";	// This selects the one table we want (but didn't always work)
...
{
	if($table['Name'] == CLAN_FORUM_THREAD)
	{
...
$query = "
SELECT ft.thread_id, ft.thread_user, ft.thread_name, ft.thread_total_replies, ft.thread_datestamp, f.forum_class, u.user_name, u.user_id FROM ".CLAN_FORUM_THREAD." as ft
...
$query = "
SELECT ft.*, f.forum_class, u.user_name, u.user_id FROM ".CLAN_FORUM_THREAD." as ft
LEFT JOIN #user AS u ON SUBSTRING_INDEX(ft.thread_user,'.',1) = u.user_id
LEFT JOIN ".CLAN_FORUM_TABLE." AS f ON f.forum_id = ft.thread_forum_id
WHERE ft.thread_parent=0
AND f.forum_class IN (".USERCLASS_LIST.")
...
$query = "
SELECT SUBSTRING_INDEX(thread_user,'.',1) AS t_user, COUNT(SUBSTRING_INDEX(ft.thread_user,'.',1)) AS ucount, u.user_name, u.user_id FROM ".CLAN_FORUM_THREAD." as ft
LEFT JOIN #user AS u ON SUBSTRING_INDEX(ft.thread_user,'.',1) = u.user_id
WHERE ft.thread_parent=0
GROUP BY t_user
...
foreach($most_activeArray as $ma)
{
	...
	<td style='width: 40%;' class='forumheader3'><a href='".CLAN_FORUM_APP."forum_viewtopic.php?{$ma['thread_id']}'>{$ma['thread_name']}</a></td>
	...
	<td style='width: 20%; text-align: center;' class='forumheader3'>".$gen->convert_date($ma['thread_datestamp'], CLAN_FORUM_TIMEFORMAT)."</td>
	</tr>
	";
	$count++;
}
