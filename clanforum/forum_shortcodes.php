...
include_once(CLAN_FORUM_APP.'handlers/shortcode_handler.php');
...
return LAN_06.': '.$gen->convert_date($post_info['user_join'], CLAN_FORUM_TIMEFORMAT).'<br />';
}
SC_END
	
SC_BEGIN THREADDATESTAMP
global $post_info, $gen, $thread_id;
return "<a id='post_{$post_info['thread_id']}' href='".e_SELF."?{$post_info['thread_id']}.post'>".IMAGE_post."</a> ".$gen->convert_date($post_info['thread_datestamp'], CLAN_FORUM_TIMEFORMAT);
SC_END
...
global  $post_info, $tp;
if(e107::isInstalled('pm')  && ($post_info['user_id'] > 0))
{
...
$avatar = e107::getParser()->parseTemplate("{USER_AVATAR=".$post_info['user_image']."}",true);
 
return "<div class='spacer'>".$avatar."></div>";
} else {
return "";
}
} else {
...
if ($post_info['user_forums'] >= $plugPref['forum_posts_sig'] && check_class($plugPref['forum_class_sig'], $post_info['user_class'])) {
...
return "<a href='".CLAN_FORUM_APP."forum_post.php?quote.{$post_info['thread_id']}'>".IMAGE_quote."</a>";
}
SC_END
...
if (USER) {
return "<a href='".CLAN_FORUM_APP."forum_viewtopic.php?{$post_info['thread_id']}.{$from}.report'>".IMAGE_report."</a> ";
}
SC_END
...
SC_BEGIN MEMBERID
global $post_info, $ldata,  $forum_info;
$plugPref =  e107::pref(CLAN_FORUM_FOLDER);
...
if (!array_key_exists($post_info['user_id'],$ldata)) {
	$ldata[$post_info['user_id']] = get_level($post_info['user_id'], $post_info['user_forums'], $post_info['user_comments'], $post_info['user_chats'], $post_info['user_visits'], $post_info['user_join'], $post_info['user_admin'], $post_info['user_perms'], $plugPref, $fmod);
}
...
return $gen->convert_date($post_info['thread_edit_datestamp'],CLAN_FORUM_TIMEFORMAT);
}
return "";
SC_END
