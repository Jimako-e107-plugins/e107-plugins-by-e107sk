...
include_lan(CLAN_FORUM_LANGUAGES.'lan_forum_viewtopic.php');
...
{
	header("location:".CLAN_FORUM_APP."forum_viewforum.php?".$_POST['forumjump']);
	exit;
}
...
if (!e_QUERY)
{
	...
	header("Location:".CLAN_FORUM_APP."forum.php");
	exit;
}
else
{
...
if($topic_from === 'post')
{
	if($thread_id)
	{
		...
		$pages = ceil(($post_num['post_num']+1)/$plugPref['forum_postspage']);
		$topic_from = ($pages-1) * $plugPref['forum_postspage'];
...
require_once(CLAN_FORUM_APP.'forum_shortcodes.php');
...
if ($action == "report") {
	...
	if (isset($_POST['report_thread'])) {
		...
		if ($plugPref['reported_post_email']) {
		...
		$thread_name = $oldtp -> toHTML($thread_info['head']['thread_name'], TRUE, 'USER_TITLE');
		define("e_PAGETITLE", LAN_01." / ".LAN_426." ".$thread_name);
		require_once(HEADERF);
		$text = "<form action='".CLAN_FORUM_APP."forum_viewtopic.php?".e_QUERY."' method='post'> <table style='width:100%'>
			<tr>
			<td  style='width:50%' >
			".LAN_415.": ".$thread_name." <a href='".CLAN_FORUM_APP."forum_viewtopic.php?".$thread_id.".post'><span class='smalltext'>".LAN_420." </span>
			</a>
			</td>
...
$pm_installed = ($plugPref['pm_title'] ? TRUE : FALSE);

$replies = $forum->thread_count($thread_id)-1;
if ($topic_from === 'last') {
	$plugPref['forum_postspage'] = ($plugPref['forum_postspage'] ? $plugPref['forum_postspage'] : 10);
	$pages = ceil(($replies+1)/$plugPref['forum_postspage']);
	$topic_from = ($pages-1) * $plugPref['forum_postspage'];
}
$gen = new convert;
$thread_info = $forum->thread_get($thread_id, $topic_from-1, $plugPref['forum_postspage']);
...
define("e_PAGETITLE", LAN_01." / ".$oldtp->toHTML($forum_info['forum_name'], TRUE, 'USER_TITLE')." / ".$oldtp->toHTML($thread_info['head']['thread_name'], TRUE, 'USER_TITLE'));
...
if (MODERATOR)
{
	if ($_POST)
	{
		require_once(CLAN_FORUM_APP.'forum_mod.php');
...
require_once(HEADERF);
require_once(CLAN_FORUM_APP."handlers/level_handler.php");
if ($message)
{
...
if (stristr($thread_info['head']['thread_name'], "[".LAN_430."]"))
{
	...
	$poll = new poll;
	$pollstr = "<div class='spacer'>".$poll->render_poll($_qry, CLAN_FORUM_TABLE_NOPREFIX, "query", TRUE)."</div>";
}
//Load forum templates
    
if (!$FORUMSTART) {
	if (file_exists(CLAN_FORUM_TEMPLATES."forum_viewtopic_template.php"))
	{
 
		require_once(CLAN_FORUM_TEMPLATES."forum_viewtopic_template.php");
	}
	else if (file_exists(CLAN_FORUM_TEMPLATES."forum_template.php"))
	{
		require_once(CLAN_FORUM_TEMPLATES."forum_template.php");
	}
	else
	{
		require_once(CLAN_FORUM_APP."templates/forum_viewtopic_template.php");
	}
}

$forum_info['forum_name'] = $oldtp -> toHTML($forum_info['forum_name'], TRUE,'USER_TITLE');

// get info for main thread -------------------------------------------------------------------------------------------------------------------------------------------------------------------

$forum->set_crumb(TRUE); // Set $BREADCRUMB (and BACKLINK)
$THREADNAME = $oldtp->toHTML($thread_info['head']['thread_name'], TRUE, 'USER_TITLE');
...
if ($pages > 1)
{
	$parms = ($replies+1).",{$plugPref['forum_postspage']},{$topic_from},".e_SELF.'?'.$thread_id.'.[FROM],off';
	$GOTOPAGES = $oldtp->parseTemplate("{NEXTPREV={$parms}}");
}
...
if ((check_class($forum_info['forum_postclass']) && check_class($forum_info['parent_postclass'])) || MODERATOR)
{
	...
	{
		$BUTTONS = "<a href='".CLAN_FORUM_APP."forum_post.php?rp.".e_QUERY."'>".IMAGE_reply."</a>";
	}
	$BUTTONS .= "<a href='".CLAN_FORUM_APP."forum_post.php?nt.".$forum_info['forum_id']."'>".IMAGE_newthread."</a>";
}
...
$forstr = $oldtp->simpleParse($FORUMSTART);
...
for($i = 0; $i < count($thread_info)-1; $i++)
{
	...
	$e_hide_query = "SELECT thread_id FROM ".CLAN_FORUM_THREAD." WHERE (`thread_parent` = {$thread_id} OR `thread_id` = {$thread_id}) AND SUBSTRING_INDEX(thread_user,'.',1) = ".USERID;
	$e_hide_hidden = FORLAN_HIDDEN;
	$e_hide_allowed = USER;

	//fix for not parsed user extended fields 
  $fieldsvar['USER_EXTENDED_LOCATION']   = $tp->parseTemplate("{USER_EXTENDED=location.value.".$loop_uid."}", true);
  if($fieldsvar['USER_EXTENDED_LOCATION']) {
	 $fieldsvar['USER_EXTENDED_LOCATION']  = $tp->parseTemplate("{USER_EXTENDED=location.text.".$loop_uid."}: ".$fieldsvar['USER_EXTENDED_LOCATION']."<br />", true);
	}
	
  $fieldsvar['USER_EXTENDED_BIRTHDAY']   = $tp->parseTemplate("{USER_EXTENDED=birthday.value.".$loop_uid."}", true);
  if($fieldsvar['USER_EXTENDED_BIRTHDAY']) {
	 $fieldsvar['USER_EXTENDED_BIRTHDAY']  = $tp->parseTemplate("{USER_EXTENDED=birthday.text.".$loop_uid."}: ".$fieldsvar['USER_EXTENDED_BIRTHDAY']."<br />", true);
	}
	...
	if($post_info['thread_parent'])
	{
		...
		{
			$tempforthr = $oldtp->parseTemplate($FORUMREPLYSTYLE_ALT, TRUE, $forum_shortcodes)."\n";
			$forrep .= e107::getParser()->simpleParse($tempforthr, $fieldsvar)."\n";
		}
		else
		{
		  
		  $tempforthr = $oldtp->parseTemplate($FORUMTHREADSTYLE, TRUE, $forum_shortcodes)."\n";
		  $forthr = e107::getParser()->simpleParse($tempforthr, $fieldsvar);
...
$forend = $oldtp->simpleParse($FORUMEND);
...
echo "<script type=\"text/javascript\">
	...
	if (mode == 'thread') {
	return confirm(\"".$oldtp->toJS(LAN_409)."\");
	} else {
	return confirm(\"".$oldtp->toJS(LAN_410)." [ ".$oldtp->toJS(LAN_411)."\" + thread + \" ]\");
	}
	}
	</script>";
require_once(FOOTERF);
...
function showmodoptions()
{
	...
	{
		$type = 'thread';
		$ret = "<form method='post' action='".CLAN_FORUM_APP."forum_viewforum.php?{$forum_id}' id='frmMod_{$forum_id}_{$post_info['thread_id']}'>";
	}
	else
	{
		$type = 'reply';
	...
	$ret .= "
		<div>
		<a href='".CLAN_FORUM_APP."forum_post.php?edit.{$post_info['thread_id']}.{$topic_from}'>".IMAGE_admin_edit."</a>
	...
	if ($type == 'thread')
	{
		$ret .= "<a href='".CLAN_FORUM_APP."forum_conf.php?move.{$thread_id}'>".IMAGE_admin_move2."</a>";
	}
	$ret .= "
		</div>
		</form>";
...
function rpg($user_join, $user_forums)
{
	...
	{
		$bar_image = CLAN_FORUM_APP."images/".IMODE."/bar.jpg";
	}
