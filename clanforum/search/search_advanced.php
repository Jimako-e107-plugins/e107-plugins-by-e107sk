...
$advanced[CLAN_FORUM_FOLDER]['type'] = 'dropdown';
$advanced[CLAN_FORUM_FOLDER]['text'] = FOR_SCH_LAN_2.':';
$advanced[CLAN_FORUM_FOLDER]['list'][] = array('id' => 'all', 'title' => FOR_SCH_LAN_3);

$advanced_caption['id'] = CLAN_FORUM_FOLDER;
$advanced_caption['title']['all'] = FOR_SCH_LAN_3;

if ($sql -> gen("SELECT f.forum_id, f.forum_name FROM ".CLAN_FORUM_TABLE." AS f LEFT JOIN ".CLAN_FORUM_TABLE." AS fp ON fp.forum_id = f.forum_parent WHERE f.forum_parent != 0 AND fp.forum_class IN (".USERCLASS_LIST.") AND f.forum_class IN (".USERCLASS_LIST.")")) {
	while ($row = $sql -> fetch()) {
		$advanced[CLAN_FORUM_FOLDER]['list'][] = array('id' => $row['forum_id'], 'title' => $row['forum_name']);
