...
// advanced
$advanced_where = "";
if (isset($_GET[CLAN_FORUM_FOLDER]) && is_numeric($_GET[CLAN_FORUM_FOLDER])) {
	$advanced_where .= " f.forum_id='".$_GET[CLAN_FORUM_FOLDER]."' AND";
}
...
function search_forum($row) {
	...
	$res['link'] = CLAN_FORUM_APP."forum_viewtopic.php?".$link_id.".post";
	$res['pre_title'] = $title ? FOR_SCH_LAN_5.": " : "";
	$res['title'] = $title ? $title : LAN_SEARCH_9;
	$res['pre_summary'] = "<div class='smalltext' style='padding: 2px 0px'><a href='".CLAN_FORUM_APP."forum.php'>".FOR_SCH_LAN_1."</a> -> <a href='".CLAN_FORUM_APP."forum_viewforum.php?".$row['forum_id']."'>".$row['forum_name']."</a></div>";
