...
define("CLAN_FORUM_TEMPLATES",  THEME."templates/clanforum/" );
...
class e107clanforum
{
	...
	function e107forum()
	{
		...
		if (isset($core_pref['filter_script']) && ($core_pref['filter_script'] == 0)) $this->filterNasties = FALSE;
	}
	...
	function thread_postnum($thread_id)
	{
		...
		LEFT JOIN ".CLAN_FORUM_THREAD." AS ft ON ft.thread_parent = t.thread_parent AND ft.thread_id <= ".intval($thread_id)."
		LEFT JOIN ".CLAN_FORUM_THREAD." as fp ON fp.thread_id = t.thread_parent
		WHERE t.thread_id = ".intval($thread_id)." AND t.thread_parent != 0
		ORDER  BY ft.thread_datestamp ASC
		";
		if($ret['post_num'] = $sql->gen($query))
		{
			$row = $sql->fetch();
    ...
    $sql = e107::getDb();
		if ($type == 'thread')
		{
			...
			if ($thread_info)
			{
				...
				$sql->update(CLAN_FORUM_THREAD_NOPREFIX, "thread_lastpost = ".intval($thread_info['thread_datestamp']).", thread_lastuser = '".$tp -> toDB($thread_lastuser, true)."' WHERE thread_id = ".$id);
			}
			return $thread_info;
		}
		if ($type == CLAN_FORUM_FOLDER) {
			if ($id == 'all')
			{
				if ($sql->select(CLAN_FORUM_TABLE_NOPREFIX, 'forum_id', 'forum_parent != 0'))
				{
					while ($row = $sql->fetch())
					{
					...
					foreach($parentList as $id)
					{
						...
						$this->update_lastpost(CLAN_FORUM_TABLE_NOPREFIX, $id, $update_threads);
					}
				}
			}
			else
			{
				...
				{
					if ($sql2->db_Select(CLAN_FORUM_THREAD_NOPREFIX, 'thread_id', "thread_forum_id = $id AND thread_parent = 0"))
					{
				...
				if ($sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX, "*", "thread_forum_id={$id} ORDER BY thread_datestamp DESC LIMIT 0,1"))
				{
					$row = $sql->fetch();
				...
				$sql->update(CLAN_FORUM_TABLE_NOPREFIX, "forum_lastpost_user = '{$forum_lp_user}', forum_lastpost_info = '{$forum_lp_info}' WHERE forum_id={$id}");
			}
		}
	}
	...
	function update_subparent_lp($id)
	{
		...
		if($id == 'all')
		{
			if($sql->select(CLAN_FORUM_TABLE_NOPREFIX, 'DISTINCT(forum_sub)', 'forum_sub > 0'))
			{
				while($row = $sql->fetch())
				{
					$sublist[] = $row;
		...
		{
			$id = (int)$id;
			if($sql->select(CLAN_FORUM_TABLE_NOPREFIX, 'forum_lastpost_user, forum_lastpost_info', "forum_id = {$id} OR forum_sub = {$id} ORDER BY forum_lastpost_info DESC LIMIT 1"))
			{
				$row = $sql->fetch();
				$sql->update(CLAN_FORUM_TABLE_NOPREFIX, "forum_lastpost_user = '".$row['forum_lastpost_user']."', forum_lastpost_info ='".$row['forum_lastpost_info']."' WHERE forum_id = {$id}");
			}
		}
	}
	...
	function forum_markasread($forum_id)
	{
	  ...
	  if ($sql->select(CLAN_FORUM_THREAD_NOPREFIX, 'thread_id', $qry))
	  {
		while ($row = $sql->fetch())
		{
		  $u_new .= $row['thread_id'].".";
	...
	{
		global $sql;
		if ($sql->db_Select(CLAN_FORUM_TABLE_NOPREFIX, '*', "forum_parent=0 ORDER BY forum_order ASC"))
		{
			while ($row = $sql->fetch()) {
				$ret[] = $row;
			}
	...
	function forum_getmods($uclass = e_UC_ADMIN)
	{
		...
		else
		{
			...
			$sql->select("user", "user_id, user_name", "user_class REGEXP '{$regex}'");
		}
		while($row = $sql->fetch())
		{
	...
	function forum_getforums($type = 'all')
	{
		global $sql;
		$qry = "
		SELECT f.*, u.user_name FROM ".CLAN_FORUM_TABLE." AS f
	...
	function forum_newflag_list()
	{
	  if (!USER) return FALSE;		// Can't determine new threads for non-logged in users
		...
		$_newqry = 	"
		SELECT DISTINCT ff.forum_sub, ft.thread_forum_id FROM ".CLAN_FORUM_THREAD." AS ft
		LEFT JOIN ".CLAN_FORUM_TABLE." AS ff ON ft.thread_forum_id = ff.forum_id
	...
	function forum_get($forum_id)
	{
		...
		$forum_id = intval($forum_id);
		$qry = "
		SELECT f.*, fp.forum_class as parent_class, fp.forum_name as parent_name, fp.forum_id as parent_id, fp.forum_postclass as parent_postclass, sp.forum_name AS sub_parent FROM ".CLAN_FORUM_TABLE." AS f
		LEFT JOIN ".CLAN_FORUM_TABLE." AS fp ON fp.forum_id = f.forum_parent
		LEFT JOIN ".CLAN_FORUM_TABLE." AS sp ON f.forum_sub = sp.forum_id AND f.forum_sub > 0
	...
	function forum_get_allowed()
	{
		global $sql;
		$qry = "
		SELECT f.forum_id, f.forum_name FROM ".CLAN_FORUM_TABLE." AS f
		LEFT JOIN ".CLAN_FORUM_TABLE." AS fp ON fp.forum_id = f.forum_parent
	...
	function thread_update($thread_id, $newvals)
	{
		...
		return $sql->db_Update(CLAN_FORUM_THREAD_NOPREFIX, $newString);
	}
	...
	function forum_get_topics($forum_id, $from, $view)
	{
		...
		global $sql;
		$qry = "
		SELECT t.*, u.user_name, lpu.user_name AS lastpost_username from ".CLAN_FORUM_THREAD." as t
	...
	function thread_get_lastpost($forum_id)
	{
		$forum_id = intval($forum_id);
		global $sql;
		if ($sql->db_Count(CLAN_FORUM_THREAD_NOPREFIX, '(*)', "WHERE thread_parent = {$forum_id} "))
		{
		...
		$qry = "
		SELECT t.thread_user, t.thread_datestamp, u.user_name FROM ".CLAN_FORUM_THREAD." AS t
		LEFT JOIN #user AS u ON SUBSTRING_INDEX(t.thread_user,'.',1) = u.user_id
		{$where}
	...
	{
		return e107::getDb()->count(CLAN_FORUM_THREAD_NOPREFIX, "(*)", " WHERE thread_forum_id=".intval($forum_id)." AND thread_parent=0 ");
	}
	...
	function thread_getnext($thread_id, $forum_id, $from = 0, $limit = 100)
	{
		...
		$forum_id = intval($forum_id);
		global $sql;
		$ftab = MPREFIX.CLAN_FORUM_THREAD_NOPREFIX;
		while (!$found)
		{
			$qry = "
			SELECT t.thread_id from ".CLAN_FORUM_THREAD." AS t
	...
	function thread_get($thread_id, $start = 0, $limit = 10)
	{
		...
		$qry = "
		SELECT t.*, u.*, ue.* FROM ".CLAN_FORUM_THREAD." as t
		LEFT JOIN #user AS u
	...
	function thread_get_allowed($thread_id)
	{
		...
		$qry = "
		SELECT t.thread_id from ".CLAN_FORUM_THREAD." as t
		JOIN ".CLAN_FORUM_TABLE." AS f ON f.forum_id = t.thread_forum_id
		LEFT JOIN ".CLAN_FORUM_TABLE." AS fp ON fp.forum_id = f.forum_parent
		WHERE t.thread_id = {$thread_id}
	...
	function thread_count($thread_id)
	{
		$thread_id = intval($thread_id);
		global $sql;
		return $sql->db_Count(CLAN_FORUM_THREAD_NOPREFIX, '(*)', "WHERE thread_parent = $thread_id")+1;
	}
	...
	function thread_incview($thread_id)
	{
		...
		global $sql;
		return $sql->db_Update(CLAN_FORUM_THREAD_NOPREFIX, "thread_views=thread_views+1 WHERE thread_id=".$thread_id);
	}
	...
	function thread_get_postinfo($thread_id, $head = FALSE)
	{
		...
		$ret = array();
		$qry = "
		SELECT t.*, u.user_name, u.user_id, u.user_email from ".CLAN_FORUM_THREAD." AS t
		LEFT JOIN #user AS u
		ON SUBSTRING_INDEX(t.thread_user,'.',1) = u.user_id
		WHERE t.thread_id = $thread_id
		LIMIT 0,1
		...
		{
			$qry = "
			SELECT t.*, u.user_name, u.user_id from ".CLAN_FORUM_THREAD." AS t
			LEFT JOIN #user AS u
			ON SUBSTRING_INDEX(t.thread_user,'.',1) = u.user_id
	...
	{
		global $sql;
		$sql->db_Update(CLAN_FORUM_TABLE_NOPREFIX, "{$lp_type}={$lp_type}+1, forum_lastpost_user='{$lp_user}', forum_lastpost_info = '{$lp_info}' WHERE forum_id=".(int)$lp_forum_id);
		if($lp_forum_sub)
		{
			$sql->db_Update(CLAN_FORUM_TABLE_NOPREFIX, "forum_lastpost_user = '{$lp_user}', forum_lastpost_info = '{$lp_info}' WHERE forum_id=".(int)$lp_forum_sub);
		}
	}
    ...
    $plugPref =  e107::pref(CLAN_FORUM_FOLDER);
		...
		if ($sql->count(CLAN_FORUM_THREAD_NOPREFIX, '(*)', "WHERE thread_thread='".$thread_thread."' and thread_datestamp > ".($post_time - 180)))
		{
			return -1;
		}
		...
		$newthread_id = $sql->insert(CLAN_FORUM_THREAD_NOPREFIX, $vals, true);
		if(!$newthread_id)
		{
		...
		if ($thread_parent)
		{
			...
			$sql->db_Update(CLAN_FORUM_THREAD_NOPREFIX, "thread_lastpost={$post_time}, thread_lastuser='{$post_user}', thread_total_replies=thread_total_replies+1 WHERE thread_id = ".(int)$thread_parent);
			...
			if(!isset($plugPref['forum_eprefix']))
			{
				$plugPref['forum_eprefix'] = "[forum]";
			}

			//   Send email to originator if 'notify' set
			$email_addy = '';
			if ($plugPref['email_notify'] && $parent_thread[0]['thread_active'] == 99 && $parent_thread[0]['user_id'] != USERID)
			{
				$sql->db_Select('user', 'user_admin, user_perms, user_class, user_ban', 'user_id = '.$parent_thread[0]['user_id']);
				$thread_owner = $sql->fetch(MYSQL_ASSOC);
					...
					)
				{
					...
					sendemail($email_addy, $plugPref['forum_eprefix']." '".$thread_name."', ".LAN_381.SITENAME, $message, $email_name);
				}
			}

			// Send email to all users tracking thread - except the one that's just posted
			if ($plugPref['forum_track'] && $sql->db_Select('user', 'user_id, user_ban, user_admin, user_perms, user_class, user_email, user_name', "user_realm REGEXP('-".intval($thread_parent)."-') "))
			{
				include_once(e_HANDLER.'mail.php');
				...
				while ($tracker = $sql->fetch(MYSQL_ASSOC))
				{
					...
					{
						sendemail($tracker['user_email'], $plugPref['forum_eprefix']." '".$thread_name."', ".LAN_381.SITENAME, $message, $tracker['user_name']);
					}
				}
			}
		}
	...
	function post_getnew($count = 50, $userviewed = USERVIEWED)
	{
		...
		FROM ".CLAN_FORUM_THREAD." AS ft
		LEFT JOIN ".CLAN_FORUM_THREAD." as fp ON fp.thread_id = ft.thread_parent
		LEFT JOIN #user as u ON u.user_id = SUBSTRING_INDEX(ft.thread_user,'.',1)
		LEFT JOIN ".CLAN_FORUM_TABLE." as f ON f.forum_id = ft.thread_forum_id
...
//*** end added by marj
	...
	function forum_prune($type, $days, $forumArray)
	{
		...
		if($type == 'delete')
		{
			...
			if ($sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX, "thread_id", "thread_lastpost < $prunedate AND thread_parent=0 AND thread_s != 1 AND thread_forum_id IN ({$forumList})"))
			{
				$threadList = $sql->db_getList();
				foreach($threadList as $thread)
				{
					//Delete all replies
					$reply_count += $sql->db_Delete(CLAN_FORUM_THREAD_NOPREFIX, "thread_parent=".$thread['thread_id']);
					//Delete thread
					$thread_count += $sql->db_Delete(CLAN_FORUM_THREAD_NOPREFIX, "thread_id = ".$thread['thread_id']);
					//Delete poll if there is one
					$sql->db_Delete("poll", "poll_datestamp=".$thread['thread_id']);
				}
				foreach($forumArray as $fid)
				{
					$this->update_lastpost(CLAN_FORUM_TABLE_NOPREFIX, $fid);
		...
		{
			$pruned = $sql->db_Update(CLAN_FORUM_THREAD_NOPREFIX, "thread_active=0 WHERE thread_lastpost < $prunedate AND thread_parent=0 AND thread_forum_id IN ({$forumList})");
	...
	function forum_update_counts($forumID, $recalc_threads = false)
	{
		...
		if($forumID == 'all')
		{
			$sql->db_Select(CLAN_FORUM_TABLE_NOPREFIX, 'forum_id', 'forum_parent != 0');
		...
		$threads = $sql->db_Count(CLAN_FORUM_THREAD_NOPREFIX, "(*)", "WHERE thread_forum_id=$forumID AND thread_parent = 0");
		$replies = $sql->db_Count(CLAN_FORUM_THREAD_NOPREFIX, "(*)", "WHERE thread_forum_id=$forumID AND thread_parent != 0");
		$sql->db_Update(CLAN_FORUM_TABLE_NOPREFIX, "forum_threads='$threads', forum_replies='$replies' WHERE forum_id=".$forumID);
		if($recalc_threads == true)
		{
			$sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX, 'thread_id', "thread_forum_id = {$forumID} AND thread_parent=0");
			$tList = $sql->db_getList();
			foreach($tList as $t) {
				$sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX, 'count(*) as replies', "thread_parent = {$t['thread_id']}");
				$row = $sql->fetch(MYSQL_ASSOC);
				$sql->db_Update(CLAN_FORUM_THREAD_NOPREFIX, "thread_total_replies={$row['replies']} WHERE thread_id={$t['thread_id']}");
			}
		}
	}
	...
	function get_user_counts()
	{
		global $sql;
		$qry = "
		SELECT u.user_id AS uid, count(t.thread_user) AS cnt FROM ".CLAN_FORUM_THREAD." AS t
		LEFT JOIN #user AS u on SUBSTRING_INDEX(t.thread_user,'.',1)  = u.user_id
		WHERE u.user_id > 0
		GROUP BY uid
		";
	...
	function set_crumb($forum_href=FALSE,$thread_title="")
	{
		...
		if(is_array($FORUM_CRUMB))
		{
			...
			$replace 	= array(LAN_01, "href='".CLAN_FORUM_APP."forum.php'");
			...
			if($forum_info['sub_parent'])
			{
				...
				$replace 	= array($forum_sub_parent, "href='".CLAN_FORUM_APP."forum_viewforum.php?{$forum_info['forum_sub']}'");
			...
			$replace 	= array($tmpFname,"href='".CLAN_FORUM_APP."forum_viewforum.php?{$forum_info['forum_id']}'");
			$FORUM_CRUMB[CLAN_FORUM_FOLDER]['value'] = str_replace($search, $replace, $FORUM_CRUMB[CLAN_FORUM_FOLDER]['value']);
		...
		{
			$dfltsep = " :: ";
			$BREADCRUMB = "<a class='forumlink' href='".e_BASE."index.php'>".SITENAME."</a>".$dfltsep."<a class='forumlink' href='".CLAN_FORUM_APP."forum.php'>".LAN_01."</a>".$dfltsep;
			if($forum_info['sub_parent'])
			{
				$forum_sub_parent = (substr($forum_info['sub_parent'], 0, 1) == "*" ? substr($forum_info['sub_parent'], 1) : $forum_info['sub_parent']);
				$BREADCRUMB .= "<a class='forumlink' href='".CLAN_FORUM_APP."forum_viewforum.php?{$forum_info['forum_sub']}'>{$forum_sub_parent}</a>".$dfltsep;
			}
			...
			if ($forum_href)
			{
				$BREADCRUMB .= "<a class='forumlink' href='".CLAN_FORUM_APP."forum_viewforum.php?{$forum_info['forum_id']}'>".$tp->toHTML($tmpFname, TRUE, 'no_hook,emotes_off')."</a>";
			} else
			{
				$BREADCRUMB .= $tmpFname;
			}
...
*       $plugPref with $eMLANG_pref as its key if that pref is TRUE, default = FALSE
*
	...
	$ML = (in_array($filename,$multilang)) ? TRUE : FALSE;
		...
		else
		{
			if(defined("IMODE"))
			{
				if($ML)
				{
                	$image = (is_readable(CLAN_FORUM_APP."images/".IMODE."/".e_LANGUAGE."_".$filename)) ? e_PLUGIN_ABS."forum/images/".IMODE."/".e_LANGUAGE."_".$filename : e_PLUGIN_ABS."forum/images/".IMODE."/English_".$filename;
				}
				else
				{
			...
			else
			{
				if($ML)
				{
					$image = (is_readable(CLAN_FORUM_APP."images/lite/".e_LANGUAGE."_".$filename)) ? e_PLUGIN_ABS."forum/images/lite/".e_LANGUAGE."_".$filename : e_PLUGIN_ABS."forum/images/lite/English_".$filename;
				}
				else
                {
...
if (file_exists(CLAN_FORUM_TEMPLATES."forum_icons_template.php"))
{
	require_once(CLAN_FORUM_TEMPLATES."forum_icons_template.php");
}
else
{
	require_once(CLAN_FORUM_APP.'templates/forum_icons_template.php');
}
?>
