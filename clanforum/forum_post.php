...
|
|     $URL: https://e107.svn.sourceforge.net/svnroot/e107/trunk/e107_0.7/e107_plugins/xforum/forum_post.php $
|     $Revision: 12631 $
...
$e_wysiwyg = "post";
include_lan(CLAN_FORUM_LANGUAGES.'lan_forum_post.php');
...
require_once(CLAN_FORUM_APP."forum_post_shortcodes.php");
require_once(CLAN_FORUM_APP."forum_shortcodes.php");
require_once(e_HANDLER."ren_help.php");
$gen = new convert;
$fp = new floodprotect;
global $tp, $oldtp, $e_event;

if ($sql->select("tmp", "*", "tmp_ip='$ip' ")) {
	$row = $sql->fetch();
...
$forum_info['forum_name'] = $oldtp -> toHTML($forum_info['forum_name'], TRUE);
...
if (isset($_POST['submitpoll']))
{
	...
	if (!$FORUMPOST)
	{
		if (file_exists(CLAN_FORUM_TEMPLATES."forum_posted_template.php"))
		{
			require_once(CLAN_FORUM_TEMPLATES."forum_posted_template.php");
		}
		else
		{
			require_once(CLAN_FORUM_APP."templates/forum_posted_template.php");
		}
	}
...
if (isset($_POST['fpreview']))
{
	...
	$postdate = $gen->convert_date(time(), CLAN_FORUM_TIMEFORMAT);
	$tsubject = $oldtp->post_toHTML($_POST['subject'], true);
	$tpost = $oldtp->post_toHTML($_POST['post'], true);

	if ($_POST['poll_title'] != "" && $plugPref['forum_poll'])
	{
		require_once(e_PLUGIN."poll/poll_class.php");
		$poll = new poll;
		$poll->render_poll($_POST, CLAN_FORUM_TABLE_NOPREFIX, "notvoted");
	}

	if (!$FORUM_PREVIEW)
	{
		if (file_exists(THEME."templates/forum_preview_template.php"))
		{
			require_once(THEME."templates/forum_preview_template.php");
		}
		else
		{
			require_once(CLAN_FORUM_APP."templates/forum_preview_template.php");
		}
	}
	...
	$anonname = $oldtp->post_toHTML($_POST['anonname'], FALSE);

  	$post = $oldtp->post_toForm($_POST['post']); 
	$subject = $oldtp->post_toHTML($_POST['subject'], false);
...
if (isset($_POST['newthread']) || isset($_POST['reply']))
{
	...
	{
		if ($fp->flood(CLAN_FORUM_THREAD_NOPREFIX, "thread_datestamp") == FALSE && !ADMIN)
		{
		...
		else
		{
			...
			if ($poster == -1)
			{
				...
				{
					$tmpdata = "reply.".$tp -> toDB($_POST['anonname']).".".$tp -> toDB($_POST['subject']).".".$oldtp -> toDB($_POST['post']);
				}
				else
				{
					$tmpdata = "newthread^".$tp -> toDB($_POST['anonname'])."^".$tp -> toDB($_POST['subject'])."^".$tp -> toDB($_POST['post']);
				}
				$sql->insert("tmp", "'$ip', '".time()."', '$tmpdata' ");
				loginf();
				require_once(FOOTERF);
				exit;
			}
		...
		if ($plugPref['forum_redirect'])
		{
			redirect(CLAN_FORUM_APP."forum_viewtopic.php?{$iid}.last");
		}
		else
		{
			require_once(HEADERF);     
			if (!$FORUMPOST)
			{
				if (file_exists(THEME."templates/clanforum/forum_posted_template.php"))
				{
					require_once(THEME."templates/clanforum/forum_posted_template.php");
				}
				else
				{
					require_once(CLAN_FORUM_APP."templates/forum_posted_template.php");
...
if (isset($_POST['update_thread']))
{
	...
	else
	{
		...
		$e107cache->clear("newforumposts");
		$url = CLAN_FORUM_APP."forum_viewtopic.php?{$thread_info['head']['thread_id']}.0";
...
if (isset($_POST['update_reply']))
{
	...
	else
	{
		...
		$url = CLAN_FORUM_APP."forum_viewtopic.php?{$id}.post";
		echo "<script type='text/javascript'>document.location.href='".$url."'</script>\n";
		$newvals['thread_edit_datestamp'] = time();
		$newvals['thread_thread'] = $_POST['post'];
		$forum->thread_update($id, $newvals);
...
if (!$FORUMPOST)
{
	if (is_readable(THEME."templates/clanforum/forum_post_template.php"))
	{
		include_once(THEME."templates/clanforum/forum_post_template.php");
	}
	else
	{
		include_once(CLAN_FORUM_APP."templates/forum_post_template.php");
	}
}
if (!$FORUMPOST) 
/* check post access (bugtracker #1424) */

if($action == "rp" && !$sql -> select(CLAN_FORUM_THREAD_NOPREFIX, "*", "thread_id='{$id}'"))
{
	$ns -> tablerender(LAN_20, "<div style='text-align:center'>".LAN_399."</div>");
	require_once(FOOTERF);
	exit;
...
elseif($action == "nt")
{
	...
	if (!$sact && !$sql -> select(CLAN_FORUM_TABLE_NOPREFIX, "*", "forum_id='{$id}'" ))
	{
    $ns -> tablerender(LAN_20, "<div style='text-align:center'>".LAN_399."</div>");
		require_once(FOOTERF);
		exit;
...
else
{
  ...
  // DB access should pass - after all, the thread should exist
	$sql->gen("SELECT t.*, p.forum_postclass FROM ".CLAN_FORUM_THREAD." AS t 
	LEFT JOIN ".CLAN_FORUM_TABLE." AS p ON t.thread_forum_id=p.forum_id WHERE thread_id='{$id}'");
	$fpr = $sql -> fetch();
...
$text = $oldtp->parseTemplate($FORUMPOST, FALSE, $forum_post_shortcodes);
 
// -------------------------------------------------------------------------------------------------------------------------------------------------------------

if ($plugPref['forum_enclose'])
{
	$ns->tablerender($plugPref['forum_title'], $text);
}
else
{
	echo $text;
...
//		$name = "0.".LAN_311.chr(1).$ip;
	...
	{
		if ($sql->select("user", "user_id, user_ip", "user_name='".$tp -> toDB($name)."'"))
		{
			$row = $sql->fetch();
			if ($row['user_ip'] == $ip)
			{
...
{
	global  $forum_info, $thread_info, $admin_log;
		...
		if ($uploaded = file_upload('/'.e_FILE."public/", "attachment", "FT{$tid}_"))
		{
			foreach($uploaded as $upload)
			{
			  if ($upload['error'] == 0)
			  {
				...
				{
					if(isset($plugPref['forum_maxwidth']) && $plugPref['forum_maxwidth'] > 0)
					{
						require_once(e_HANDLER."resize_handler.php");
						$orig_file = $upload['name'];
						$p = strrpos($orig_file,'.');
						$new_file = substr($orig_file, 0 , $p)."_".substr($orig_file, $p);
						if(resize_image(e_FILE.'public/'.$orig_file, e_FILE.'public/'.$new_file, $plugPref['forum_maxwidth']))
						{
							if($plugPref['forum_linkimg'])
							{
