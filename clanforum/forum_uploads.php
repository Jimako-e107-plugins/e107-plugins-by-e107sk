...
include_lan(CLAN_FORUM_LANGUAGES.'lan_forum_uploads.php');
...
if($sql->db_Select(CLAN_FORUM_THREAD_NOPREFIX,'thread_id, thread_thread, thread_parent', "thread_thread REGEXP '.*_".USERID."_FT.*'"))
{
	...
	foreach($fileList as $finfo)
	{
		if($finfo['fname'])
		{
			...
			if($found != FALSE)
			{
				...
				{
					$txt .= "<td class='forumheader3'>".FRMUP_7.": <a href='".CLAN_FORUM_APP."forum_viewtopic.php?{$tinfo['thread_id']}.post'>{$tinfo['thread_parent']}</a></td>";
				}
				else
				{
					$txt .= "<td class='forumheader3'>".FRMUP_7.": <a href='".CLAN_FORUM_APP."forum_viewtopic.php?{$tinfo['thread_id']}'>{$tinfo['thread_id']}</a></td>";
				}	
