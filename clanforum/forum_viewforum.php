...
|
|     $URL: https://e107.svn.sourceforge.net/svnroot/e107/trunk/e107_0.7/e107_plugins/xforum/forum_viewforum.php $
...
include_lan(CLAN_FORUM_LANGUAGES.'lan_forum_viewforum.php');

if (!e_QUERY)
{
	js_location(CLAN_FORUM_APP."forum.php");
}
else
{
	$tmp = explode(".", e_QUERY);
...
if (!$FORUM_VIEW_START) {
	if (file_exists(CLAN_FORUM_TEMPLATES."forum_viewforum_template.php"))
	{
		require_once(CLAN_FORUM_TEMPLATES."forum_viewforum_template.php");
	}
	else
	{
		require_once(CLAN_FORUM_APP."templates/forum_viewforum_template.php");
	}
}
...
{              
	$NEWTHREADBUTTON = "<a href='".CLAN_FORUM_APP."forum_post.php?nt.".$forum_id."'>".IMAGE_newthread."</a>";
}
...
$SEARCH = "
	...
	<input type='hidden' name='ref' value='".CLAN_FORUM_TABLE_NOPREFIX."' />
...
if (count($thread_list) )
{
	...
	foreach($thread_list as $thread_info) {
		...
		if ($sticky_threads > 0 && !$stuck && $plugPref['forum_hilightsticky'])
		{
...
e107::getDb()->select(CLAN_FORUM_TABLE_NOPREFIX, "*", "forum_parent !=0 AND forum_class!='255' ");
$FORUMJUMP = forumjump();
...
//$forum_view_start = pregx_replace("/\{(.*?)\}/e", '$\1', $FORUM_VIEW_START);
//$forum_view_start = $tp->simpleParse($FORUM_VIEW_START);     
		$fanat1k_tmp = $FORUM_VIEW_START;				
		preg_match_all("/\{(.*?)\}/", $fanat1k_tmp, $matches);				
				for($i = 0; $i < count($matches[1]); $i++){				
							        $fanat1k_tmp = str_replace("{{$matches[1][$i]}}", ${$matches[1][$i]}, $fanat1k_tmp);				
							}			
		$forum_view_start =  $fanat1k_tmp; 
		

//$forum_view_end = pregx_replace("/\{(.*?)\}/e", '$\1', $FORUM_VIEW_END);
//$forum_view_end = $tp->simpleParse($FORUM_VIEW_END);
		$fanat1k_tmp = $FORUM_VIEW_END;				
		preg_match_all("/\{(.*?)\}/", $fanat1k_tmp, $matches);				
				for($i = 0; $i < count($matches[1]); $i++){				
							        $fanat1k_tmp = str_replace("{{$matches[1][$i]}}", ${$matches[1][$i]}, $fanat1k_tmp);				
							}			
		$forum_view_end =  $fanat1k_tmp; 
 
if ($plugPref['forum_enclose'])
{       
	$ns->tablerender($plugPref['forum_title'], $forum_view_start.$forum_view_subs.$forum_view_forum.$forum_view_end, array('forum_viewforum', 'main1'));
}
else
{        
...
{
	global $forum, $tp, $current_row, $FORUM_VIEW_FORUM, $FORUM_VIEW_FORUM_STICKY, $FORUM_VIEW_FORUM_ANNOUNCE, $gen, $plugPref, $forum_id, $menu_pref;
	$text = "";
	...
	$tip_length = ($plugPref['forum_tiplength'] ? $plugPref['forum_tiplength'] : 200);
	...
	if ($REPLIES)
	{
		$lastpost_datestamp = $gen->convert_date($thread_info['thread_lastpost'], CLAN_FORUM_TIMEFORMAT);
	...
	$THREADDATE = $gen->convert_date($thread_info['thread_datestamp'], CLAN_FORUM_TIMEFORMAT);
	$ICON = ($newflag ? IMAGE_new : IMAGE_nonew);
	if ($REPLIES >= $plugPref['forum_popular'])
	{
	...
	$THREADNAME = "<a {$title} href='".CLAN_FORUM_APP."forum_viewtopic.php?{$thread_info['thread_id']}'>{$thread_name}</a>";
	...
	if ($pages > 1)
	{
		if($pages > 6)
		{
			...
			{
				$PAGES .= $PAGES ? " " : "";
				$PAGES .= "<a href='".CLAN_FORUM_APP."forum_viewtopic.php?".$thread_info['thread_id'].".".($a * $plugPref['forum_postspage'])."'>".($a+1)."</a>";
			}
			$PAGES .= " ... ";
	...
	if (MODERATOR)
	{
		...
		$ADMIN_ICONS .= "<a href='".CLAN_FORUM_APP."forum_conf.php?move.".$thread_id."'>".IMAGE_admin_move."</a>";
		$ADMIN_ICONS .= "
			</div></form>
			";
	}
	...
	if ($thread_info['thread_s'] == 1 && $F_VIEW_FORUM_STICKY)
	{
		...
		//return(pregx_replace("/\{(.*?)\}/e", '$\1', $F_VIEW_FORUM_STICKY));
		$fanat1k_tmp = $F_VIEW_FORUM_STICKY;		
		preg_match_all("/\{(.*?)\}/", $fanat1k_tmp, $matches);				
		for($i = 0; $i < count($matches[1]); $i++){				
		$fanat1k_tmp = str_replace("{{$matches[1][$i]}}", ${$matches[1][$i]}, $fanat1k_tmp);				
		}		
	...
	{
		//return(pregx_replace("/\{(.*?)\}/e", '$\1', $F_VIEW_FORUM_ANNOUNCE));
		$fanat1k_tmp = $F_VIEW_FORUM_ANNOUNCE;		
		preg_match_all("/\{(.*?)\}/", $fanat1k_tmp, $matches);				
		for($i = 0; $i < count($matches[1]); $i++){				
		$fanat1k_tmp = str_replace("{{$matches[1][$i]}}", ${$matches[1][$i]}, $fanat1k_tmp);				
		}		
	...
	//return(pregx_replace("/\{(.*?)\}/e", '$\1', $F_VIEW_FORUM));
	$fanat1k_tmp = $F_VIEW_FORUM;		
	preg_match_all("/\{(.*?)\}/", $fanat1k_tmp, $matches);				
	for($i = 0; $i < count($matches[1]); $i++){				
	$fanat1k_tmp = str_replace("{{$matches[1][$i]}}", ${$matches[1][$i]}, $fanat1k_tmp);				
	}		
...
function parse_sub($subInfo)
{
	...
	$SUB_FORUMTITLE = "<a href='".CLAN_FORUM_APP."forum_viewforum.php?{$subInfo['forum_id']}'>{$subInfo['forum_name']}</a>";
	...
	if($subInfo['forum_lastpost_info'])
	{
		...
		$lp_thread = "<a href='".CLAN_FORUM_APP."forum_viewtopic.php?{$tmp[1]}.last'>".IMAGE_post2."</a>";
		$lp_date = $gen->convert_date($tmp[0], CLAN_FORUM_TIMEFORMAT);
