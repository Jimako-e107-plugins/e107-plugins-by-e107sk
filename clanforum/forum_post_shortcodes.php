...
if(!e_WYSIWYG)
{
	$ret .= display_help('helpb', CLAN_FORUM_FOLDER);
}
return $ret;
SC_END
...
SC_BEGIN FILEATTACH
global  $fileattach, $fileattach_alert;
$plugPref =  e107::pref(CLAN_FORUM_FOLDER);
if ($plugPref['forum_attach'] && strpos(e_QUERY, "edit") === FALSE && (check_class($plugPref['upload_class']) || getperms('0')))
{
	...
	else
	{
		...
		if(ADMIN)
		{
			if(!$fileattach_alert)
			{
				$fileattach_alert = "<tr><td colspan='2' class='nforumcaption2'>".($plugPref['image_post'] ? LAN_390 : LAN_416)."</td></tr><tr><td colspan='2' class='forumheader3'>".LAN_FORUM_1."</td></tr>\n";
			}
...
global  $thread_info, $action;
$plugPref =  e107::pref(CLAN_FORUM_FOLDER);
if ($plugPref['email_notify'] && $action == "nt" && USER)
{
	...
	else
	{
		...
		{
			$chk = ($plugPref['email_notify_on'] ? "checked='checked'" : "");
		}
	}
...
SC_BEGIN POLL
global $poll_form, $action;
$plugPref =  e107::pref(CLAN_FORUM_FOLDER);
if ($action == "nt" && $plugPref['forum_poll'] && strpos(e_QUERY, "edit") === FALSE)
{
	return $poll_form;
}
return "";
