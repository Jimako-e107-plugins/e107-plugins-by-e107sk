...
$email = $tp->toDB($_GET['email']);

if($react == 1){
	//Reactivate
	$result = $sql->update("clan_wars_mail", "active='1' WHERE member='".USERID."'");
	if($result){
		print '1';
	}	
}else{
	if($sql->count("clan_wars_mail", "(*)", "WHERE member='".USERID."'") > 0){
		die('userinlist');
	}if($sql->count("clan_wars_mail", "(*)", "WHERE email='$email'") > 0){
		die('emailinlist');
	}
	//subscribe
	if($conf['emailact']){
		...
		$result = $sql->insert("clan_wars_mail", array("member" => USERID, "email" => $email, "active" => 0, "code" => $code, "subscrtime" => time()));	
		if(intval($result) > 0){
			//EMAIL
			$pageURL = 'http';
		...
		$result = $sql->insert("clan_wars_mail", array("member" => USERID, "email" => $email, "active" => 1, "subscrtime" => time()));	
		if(intval($result) > 0){
			print '1';
		}
	}
