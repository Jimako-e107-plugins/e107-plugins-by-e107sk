wars_jq().ready(function() {
	wars_jq("#opp_tag").autocomplete('admin_old.php?Search&type=opp_tag', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	}).result(function(){OppFields('tag');});
	wars_jq("#opp_name").autocomplete('admin_old.php?Search&type=opp_name', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	}).result(function(){OppFields('name');});
	wars_jq("#opp_url").autocomplete('admin_old.php?Search&type=opp_url', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	});
	wars_jq("#style").autocomplete('admin_old.php?Search&type=style', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	});
	wars_jq("#mapname").autocomplete('admin_old.php?Search&type=mapname&gid='+gid, {
		matchContains: true,
		selectFirst: false,
		mustMatch: mapmustmatch,
		minChars: 1
	});
	wars_jq("#gametype").autocomplete('admin_old.php?Search&type=gametype', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	});
	wars_jq("#playername0").autocomplete('admin_old.php?Search&type=player', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	});
	wars_jq("#playername1").autocomplete('admin_old.php?Search&type=player', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	}) ;
	wars_jq("#playername2").autocomplete('admin_old.php?Search&type=player', {
		matchContains: true,
		selectFirst: false,
		minChars: 1
	});
...
function OppFields(type){
    ...
    http.open("GET","admin_old.php?GetInfo&type=" + escape(type) + "&param=" + escape(param)+"&code="+Math.floor(Math.random()*99999),true);
...
function AddAutoComplete(id){
	wars_jq().ready(function() {
		wars_jq("#mapname"+id).autocomplete('admin_old.php?Search&type=mapname&gid='+gid, {
			matchContains: true,
			selectFirst: false,
			mustMatch: mapmustmatch,
			minChars: 1
		});
		wars_jq("#gametype"+id).autocomplete('admin_old.php?Search&type=gametype', {
			matchContains: true,
			selectFirst: false,
			minChars: 1
		});
...
function AddMap() {
    ...
    http.open("GET","admin_old.php?AddMap&mapname=" + escape(mapname) + "&gametype=" + escape(gametype) + "&our_score=" + parseInt(our_score) + "&opp_score=" + parseInt(opp_score) +"&wid=" + wid +"&gid=" + gid+"&code="+Math.floor(Math.random()*99999),true);
        ...
        if(http.readyState == 4) {
            if(http.status == 200) {
                ...
                if(result > 0) {
					...
					newcontent += '<td width="118"  style="text-align:right;" class="nowrap"><input type="button" class="button iconpointer" value="'+edittext+'" onclick="EditMap('+result+');">&nbsp;<input type="button" class="button iconpointer" value="'+deltext+'" onclick="DelMap('+result+');"></td> </tr> </table> </div>';
					...
					if(scorepermap == 1){
						newcontent += '<td width="36" style="text-align:right;"><input class="bginput" type="text" id="our_score'+result+'" value="'+our_score+'" style="width:50px;margin:0"></td> <td width="6" style="text-align:center;">/</td> <td width="36"><input class="bginput" type="text" id="opp_score'+result+'" value="'+opp_score+'" style="width:50px;margin:0"></td>';
					}
					newcontent += '<td width="118" style="text-align:right;" class="nowrap"><input type="button" class="button iconpointer" value="'+savetext+'" onclick="SaveMap('+result+');">&nbsp;<input type="button" class="button iconpointer" value="'+canceltext+'" onclick="CancelMap('+result+');"></td> </tr> </table> </div>';
...
function DelMap(id) {
	...
	if(sure){
		http.open("GET","admin_old.php?DelMap&lid=" + id+"&code="+Math.floor(Math.random()*99999),true);
...
function SaveMap(id) {	
	...
	http.open("GET","admin_old.php?SaveMap&lid=" + id + "&mapname=" + escape(mapname) + "&gametype=" + escape(gametype) + "&our_score=" + parseInt(our_score) + "&opp_score=" + parseInt(opp_score)  + "&gid=" + gid + "&code="+Math.floor(Math.random()*99999),true);
...
function AddPlayer(avail) {
    ...
    //By calling this file, we have saved the data.
    http.open("GET","admin_old.php?AddPlayer&playername=" + escape(playername) + "&avail=" + parseInt(avail) + "&wid=" + parseInt(wid) + "&warstatus="+ parseInt(warstatus)+"&code="+Math.floor(Math.random()*99999),true);
...
function DelPlayer(id,avail) {
	...
	if(sure){
		http.open("GET","admin_old.php?DelPlayer&pid=" + id+"&code="+Math.floor(Math.random()*99999),true);
...
function DelScreen(id) {
	...
	if(sure){
		http.open("GET","admin_old.php?DelScreen&sid=" + id+"&code="+Math.floor(Math.random()*99999),true);
...
function DelComment(id) {
	...
	if(sure){
		http.open("GET","admin_old.php?DelComment&cid=" + id+"&code="+Math.floor(Math.random()*99999),true);
...
function DelWarComments() {
	...
	if(sure){
		http.open("GET","admin_old.php?DelWarComments&wid=" + wid+"&code="+Math.floor(Math.random()*99999),true);
...
function SaveComment(id) {	
	...
	http.open("GET","admin_old.php?SaveComment&cid=" + id + "&comment=" + escape(comment)+"&code="+Math.floor(Math.random()*99999),true);
