...
if (!defined('WARS_ADMIN') or !preg_match("/admin_old\.php\?Teams/i", $_SERVER['REQUEST_URI'])) {
	die ("Access Denied");
}
$teams = e107::getDb()->count("clan_teams");
	...
	<?php
	$text = "<div class='text-center'><form action='admin_old.php?AddTeam' method='post' enctype='multipart/form-data' onSubmit='return CheckForm();'>
		<table id='addteamtable' class='table adminform' >
			<tr>
...
if($teams>0){
	...
	$text = "<form method='post' action='admin_old.php?assignteams' name='assignteams'>";
		$text .= "<table id='teamstable' class='table adminform'>";	
		$text .= "<tr>
		...
		$sql -> db_Select("clan_teams", "*", "ORDER BY ".(isset($pref['plug_installed']['clanmembers'])?"position":"team_name")." ASC", "");
			...
			$text .= "<tr id='$tid'>
				...
				if(isset($pref['plug_installed']['clanmembers'])){
					...
					$text .= "<td class='forumheader3'   nowrap><input type='button' class='button' value='"._WEDIT."' onclick=\"window.location='admin_old.php?EditTeam&tid=$tid'\">&nbsp;<input type='button' class='button' value='"._WDEL."' onclick=\"DelTeam('$tid');\"></td>
				</tr>";
			}
	$text .= "</tbody>
	</table>";
