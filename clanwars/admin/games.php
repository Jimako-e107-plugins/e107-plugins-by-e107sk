...
if (!defined('WARS_ADMIN') or !preg_match("/admin_old\.php\?Games/i", $_SERVER['REQUEST_URI'])) {
	die ("Access Denied");
}
$games = e107::getDb()->count("clan_games");
	...
	<?php
	$text = "<div class='text-center'><form action='admin_old.php?AddGame' method='post' enctype='multipart/form-data' onSubmit='return CheckForm();'>
		<table id='addgametable' class='table adminform'>
			<tr>
...
if($games>0){
	...
	$text = "<table id='gamestable' class='table adminform'>";	
		$text .= "<tr>
				...
				<tbody>";
		$games = e107::getDb()->retrieve("clan_games", "*", "ORDER BY ".(isset($pref['plug_installed']['clanmembers'])?"position":"gname")." ASC", true);
		foreach($games  as $row){
			...
			$text .= "<tr id='$gid'>
				...
				if(isset($pref['plug_installed']['clanmembers'])){
					...
					$text .= "<td class='forumheader3' nowrap><input type='button' class='button' value='"._WEDIT."' onclick=\"window.location='admin_old.php?EditGame&gid=$gid'\">&nbsp;
          <input type='button' class='button' value='"._WDEL."' onclick=\"DelGame('$gid');\"><br />
          <input type='button' class='button' value='"._MANMAPS."' onclick=\"window.location='admin_old.php?ManageMaps&gid=$gid'\" style='margin-top:3px;width:100%;' /></td>
				</tr>";
			}
	$text .= "</tbody>
	</table>";
