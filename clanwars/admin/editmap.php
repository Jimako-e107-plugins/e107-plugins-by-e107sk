...
if (!defined('WARS_ADMIN') or !preg_match("/admin_old\.php\?EditMap/i", $_SERVER['REQUEST_URI'])) {
	die ("Access Denied");
}
...
$row =  $sql->retrieve("clan_wars_maps", "*", "mid='$mid'");
 
	$mapname = $row['name'];
	$mapimage = $row['image'];

	$text =  "<form action='admin_old.php?SaveMap' method='post' enctype='multipart/form-data'>
	<table id='editgametable' class='table adminform'>
		<tr>
