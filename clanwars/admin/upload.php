...
if (!((defined('WARS_ADMIN') or defined('WARS_SPEC')) && (preg_match("/admin_old\.php\?Upload/i", $_SERVER['REQUEST_URI']) or preg_match("/clanwars\.php\?Upload/i", $_SERVER['REQUEST_URI'])))) {
...
if(ADMIN){
	$returnurl = "admin_old.php?Screens&wid=$wid";
	$thumburl = "admin_old.php?CreateThumb&wid=$wid";
}else{
	...
	if($_FILES['screenshot']['size'] > $conf['screenmaxsize'] && $conf['screenmaxsize'] > 0) { 
		...
		if($filename ==""){
			...
			if($ext != "jpg" && $ext != "jpeg" && $ext != "gif" && $ext != "png"){
				...
				$result = e107::getDb()->insert("clan_wars_screens", array("url" => $tp->toDB($screenurl), "wid" => intval($wid)));
				if(intval($result) == 0){
					?>
					<script type="text/javascript">
