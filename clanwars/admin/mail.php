...
if (!defined('WARS_ADMIN') or !preg_match("/admin_old\.php\?Mail/i", $_SERVER['REQUEST_URI'])) {
...
<?php
	...
	$text = "<div class='nowrap'>
		...
		<div id='mailsdiv'>";
	$members = array();
	$result = $sql->retrieve("clan_wars_mail", "*", " true order by member ASC", true);
			...
			$text .= "<div class='mainwrap forumheader3' id='mail$mid'>
					<table class='table adminform'>
						<tr>
							...
							<td  style='text-align:right;' nowrap><input type='button' class='iconpointer button' value='"._WEDIT."' onclick='EditMail($mid);'>&nbsp;<input type='button' class='iconpointer button' value='"._WDEL."' onclick='delData($mid);'></td>
						</tr>
					</table>
				</div>
		...
		$ns->tablerender(_WWARSMAIL, "<div class='text-center'>".$text."</div>");
?>