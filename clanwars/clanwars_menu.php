...
$totalwars = $sql->count("clan_wars", "(*)", "WHERE active='1'");	
	if($totalwars==0){
...
elseif($orderby=="ASC" && $sortby=="Players"){$ordersort="players ASC, wardate DESC";}
	...
	$upcshown = false;
	$finshown = false;
	$records = $sql->retrieve("SELECT * from #clan_wars $tables WHERE active='1' $wheresql ORDER BY ".($seperate_upcoming_finished?"status ASC,":"")." $ordersort LIMIT 0, ".$amount_of_wars, true);		 
		...
		$text .= "<tr class='forumheader3 iconpointer' title='Show Match Details' onmouseover=\"TdHover($wid);\" onmouseout=\"TdOut($wid);\" onclick=\"window.location='".e_PLUGIN ."clanwars/clanwars.php?Details&wid=$wid'\" id='war$wid'>";	
			...
			if($show_game_icon){
				$rowicon = $sql->retrieve("clan_games", "*", "gid='$game' LIMIT 1 ");
				$icon = $rowicon['icon'];
				$abbr = $rowicon['abbr'];
				$gname = $rowicon['gname'];
			...
			if($show_teams){
				$row =  $sql->retrieve("clan_teams", "*", "tid='$team' LIMIT 1 ");
